...
describe("AuthStore", () => {
  ...
  it("should update state with setAuthState", () => {
    const store = createAuthStore()
    store.setAuthState({ status: "authenticated", token: "jwt", userId: "user1", userName: "johndoe" })
    expect(store.getSnapshot()).toEqual({ status: "authenticated", token: "jwt", userId: "user1", userName: "johndoe" })
  })
  ...
  it("should handle multiple rapid state changes", () => {
    const store = createAuthStore()
    store.setAuthState({ status: "authenticated", token: "token1", userId: "user1", userName: "user1name" })
