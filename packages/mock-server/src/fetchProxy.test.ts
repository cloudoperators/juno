...
beforeEach(() => {
  global.console.warn = (...args: string[]) => {
...
describe("fetchProxy lib", () => {
  describe("fetchProxyInitDB", () => {
    it("through an warn if localDB already initialized", () => {
      ...
      fetchProxyInitDB(db)
      expect(() => fetchProxyInitDB(db)).toThrow(/localDB already initialized/)
    })
  })
  ...
  describe("fetch", () => {
    ...
    test("GET all peaks", async () => {
      const data: Response = (await fetchProxy("http://localhost:3000/peaks", {
        mock: true,
      })) as Response
