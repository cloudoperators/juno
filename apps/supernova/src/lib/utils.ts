...
export const descriptionParsed = (text: any) => {
  if (!text) return ""
 ...
 */
export const sortAlerts = (items: any) => {
  const importantSeverities = ["critical", "warning"]

  return items.sort((a: any, b: any) => {
    if (
      (a.labels?.severity === "critical" && b.labels?.severity !== "critical") ||
      (a.labels?.severity === "warning" && importantSeverities.indexOf(b.labels?.severity) < 0)
    )
      return -1
    else if (a.labels?.severity === b.labels?.severity && a.status?.state !== b.status?.state && a.status?.state)
