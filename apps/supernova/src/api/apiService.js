...
function ApiService(initialConfig) {
  ...
  const update = () => {
    if (config.fetchFn) {
      ...
      if (config?.debug) console.info(`ApiService::${config.serviceName || ""}: start fetch`)
      ...
      if (config?.debug) console.warn(`ApiService::${config.serviceName || ""}: missing fetch function`)
      return
    }
  }
  ...
  const updateWatcher = (oldConfig) => {
    ...
    if (initialFetchPerformed && oldConfig.watch === config.watch && oldConfig.watchInterval === config.watchInterval)
      return
