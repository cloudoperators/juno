...
const buildApps = async () => {
  // for each app in src/apps folder
  // create entry point
  const apps = await fs.readdir("src/apps")

  const entryPoints = apps.map((app) => `src/apps/${app}/index.js`)
...
const build = async () => {
  ...
  // build apps
  let appsCtx = await buildApps()

  // build app
  let ctx = await esbuild.context({
    ...config, // here we exclude package from bundle which are defined in peerDependencies
