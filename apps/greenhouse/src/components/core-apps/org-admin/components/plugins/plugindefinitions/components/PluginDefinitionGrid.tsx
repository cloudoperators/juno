...
const PluginDefinitionGrid: React.FC<PluginDefinitionGridProps> = (props: PluginDefinitionGridProps) => {
  return (
    <>
      <Container px={false} py>
        <div className="card-container grid gap-4 grid-cols-3">
          {props.pluginDefinitions.map((plugin) => (
            <PluginDefinitionTile key={plugin.metadata!.name!} pluginDefinition={plugin} />
          ))}
        </div>
      </Container>
    </>
