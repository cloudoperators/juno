...
export const fetchPlugins = async ({
  ...
  const allPlugins: Plugin[] = Array.isArray(response?.items) ? response.items : []

  // Filter plugins that belong to this PluginPreset
  return allPlugins.filter((plugin) => plugin.metadata?.labels?.["greenhouse.sap/pluginpreset"] === pluginPresetName)
}
