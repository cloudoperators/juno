...
import managementPluginConfig from "../adminAppsConfig.json"
...
export default (options) => {
  ...
  if (typeof configs !== "object" || Array.isArray(configs) || Object.keys(configs).length === 0) {
    configs = {}
    addMessage({
      variant: "error",
  ...
  Object.keys(configs).forEach((key) => {
    ...
    configs[key].version =
      options.environment == "qa" || options.environment == "development" ? "latest" : configs[key].version
    configs[key].props = {
      endpoint: options.apiEndpoint,
      embedded: true,
    }
    // remove attribute releaseState from the plugin config if it set and it is different from "alpha" or "beta" or "stable"
    if (configs[key].releaseState && !["alpha", "beta", "stable"].includes(configs[key].releaseState)) {
    ...
    devtools((set, get) => ({
      ...
      actions: {
        setPluginActive: (pluginId) => {
          ...
          const plugin = Object.values(get().pluginConfig).find((plugin) => plugin.name === pluginId)
          if (!plugin) return
