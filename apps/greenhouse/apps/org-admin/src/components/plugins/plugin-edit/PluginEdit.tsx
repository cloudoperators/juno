...
import { LabelSelector, PluginDefinition, PluginPreset } from "../../../../types/types"
import usePluginApi from "../plugindefinitions/hooks/usePluginApi"
import usePluginPresetApi, { PluginPresetApiResponse } from "../plugindefinitions/hooks/usePluginPresetApi"
...
const PluginEdit: React.FC<PluginEditProps> = (props: PluginEditProps) => {
  ...
  const changeIsPluginPreset = () => {
    if (isPluginPreset) {
      setEditFormState(isEditMode ? EditFormState.PLUGIN_EDIT : EditFormState.PLUGIN_CREATE)
    } else {
      if (editFormData.metadata!.labels && editFormData.metadata!.labels["greenhouse.sap/pluginpreset"]) {
  ...
  return (
    <Container px={false} py>
      ...
      {editFormData && !submitMessage.ok && (
        <>
          <Form title={editFormData.spec?.displayName ?? editFormData.metadata?.name}>
