...
const ClusterDetail: React.FC<any> = () => {
  ...
  const clusterAge = useMemo(() => {
    if (clusterDetails.cluster?.metadata?.creationTimestamp) {
      return humanizedTimePeriodToNow(clusterDetails.cluster?.metadata?.creationTimestamp)
    }
    return "unknown"
    ...
    >
      <PanelBody>
        <Container px={false} py>
          <DataGridToolbar>
            <ButtonRow>
              <Button icon="addCircle" label="Edit Cluster" onClick={() => openClusterEdit()} />
            </ButtonRow>
          </DataGridToolbar>
          <DataGrid columns={2}>
            <DataGridRow>
            ...
            <DataGridRow>
              ...
              <DataGridCell>{clusterDetails.cluster?.status?.kubernetesVersion ?? "unknown"}</DataGridCell>
            </DataGridRow>
            <DataGridRow>
              <DataGridHeadCell>Access Mode</DataGridHeadCell>
              <DataGridCell>{clusterDetails.cluster?.spec?.accessMode ?? "unknown"}</DataGridCell>
            </DataGridRow>
            {clusterDetails.clusterStatus?.message && (
              <DataGridRow>
                <DataGridHeadCell>Message</DataGridHeadCell>
                <DataGridCell>{clusterDetails.clusterStatus?.message}</DataGridCell>
              </DataGridRow>
            )}
              ...
              <DataGridRow>
                ...
                <DataGridCell>
                  <Stack gap="2" alignment="start" wrap={true}>
                    {Object.keys(clusterDetails.cluster?.metadata?.labels!).map((labelKey) => {
                      const labelValue = clusterDetails.cluster?.metadata?.labels![labelKey]
                      return (
                        <Pill
                          key={labelKey}
            ...
            {clusterDetails.cluster?.status?.statusConditions?.conditions && (
              <DataGridRow>
                ...
                <DataGridCell>
                  <ConditionList conditionArray={clusterDetails.cluster?.status?.statusConditions?.conditions} />
                </DataGridCell>
              </DataGridRow>
            )}
            {clusterDetails.plugins && <ClusterPluginList plugins={clusterDetails.plugins} />}
          </DataGrid>

          {clusterDetails.cluster?.status?.nodes && Object.keys(clusterDetails.cluster?.status?.nodes).length > 0 && (
            <Container px={false} py>
