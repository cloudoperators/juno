...
Not using AppShell to begin building a new application is possible, but comes at a relatively high price since application developers will have to include layout components, styles, context providers for styling, UI themes, portals, etc. themselves. Just starting from scratch and not adding these building blocks manually may result in the application or individual components not working at all, or not rendering or working as expected.
